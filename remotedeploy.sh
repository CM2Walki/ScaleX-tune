...
ssh $REMOTE << EOF
  ...
  docker exec '$SCALEXCONTAINERNAME' bash -c 'scalectl stop'
  docker exec '$SCALEXCONTAINERNAME' bash -c 'rm /tmp/scalectl-daemon.pid > /dev/null 2>&1'
  docker exec '$SCALEXCONTAINERNAME' bash -c 'scalectl start'
